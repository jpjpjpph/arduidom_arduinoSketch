...
#include <EEPROM.h>
    ...
    #include "DHT.h"
#endif
#define ArduiDomVersion 124 // Sers a verfifier le contenu de l'EEPROM
...
byte TimerDelayAction[CNF_NB_DPIN] = {0};
int TimerBlink[CNF_NB_DPIN] = {0};
...
void loop() {
    ...
    //
        ...
        if (SerialDataOK) { // Donnees dans le buffer Serie
            ...
            if (DataSerie[0] == 'S' && DataSerie[1] == 'P') { // ************************************************************ SP = Set Pin
                ...
                if (pinmode[pinToSet] == 'v') { // v : pulse down
                ...
                if (pinmode[pinToSet] == 'u') { // u : pulse up
                ...
                if (pinmode[pinToSet] == 'b') { // b : blinking
                    digitalWrite(pinToSet, 1);
                    check = true;
                    int pintime = 1000 * int(DataSerie[4] - '0') + 100 * int(DataSerie[5] - '0') + 10 * int(DataSerie[6] - '0') + int(DataSerie[7] - '0');
                    if (pintime > 0) {
                ...
                if (pinmode[pinToSet] == 'd') { // for customs
                    ...
                    if (pinmode[pinToSet] == 't') { // pin Radio TX
                        ...
                        /* SUPPRIME CAR SOUCIS RENCONTRES SUR FORUM
                        if (DataSerie[4] == 'H') { //// Radio Mode Chacon DIO ex:H 05580042 0100
                            // Modifs par Chevalir
    ...
    if (ReadyToSend) { // Attend que le 1er SP Soit OK
        for (int i = 0; i < CNF_NB_DPIN; i++) { // ****************************** Detection des changements de valeurs sur pins DIGITALES
            if (pinmode[i] == 'i' || pinmode[i] == 'y' || pinmode[i] == 'o' || pinmode[i] == 'h' || pinmode[i] == 'x' || pinmode[i] == 'u' || pinmode[i] == 'v') {
        ...
        //
            ...
            #if defined(FOR_BOBOX59_ONLY) //////// Convertisseur Radio sur Port ANA vers Radio 433 Virtuel
                if (digitalRead(7) == 1) {
                    if (RAZRadio == false) {
                        RAZRadio = true;
